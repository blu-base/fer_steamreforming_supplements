...
import star.common.*;
import static star.common.AMGCycleOption.Type.FLEX_CYCLE;
import static star.common.FieldFunctionTypeOption.Type.SCALAR;
import static star.common.MonitorIterationStoppingCriterionOption.Type.ASYMPTOTIC;
import static star.common.MonitorIterationStoppingCriterionOption.Type.RELATIVE_CHANGE;
import static star.common.RampCalculatorOption.Type.LINEAR_RAMP;
import static star.common.RegionManager.CreateInterfaceMode.BOUNDARY;
import static star.common.SolverStoppingCriterionLogicalOption.Type.AND;
import star.coupledflow.*;
import star.energy.*;
import static star.energy.WallThermalOption.Type.HEAT_FLUX;
import star.flow.*;
import star.keturb.*;
...
import star.species.*;
import static star.species.SpeciesSpecificationOption.Type.MOLE_FRACTION;
import star.turbulence.*;
import star.twodmesher.*;
import star.vis.*;
import static star.vis.ClipMode.NONE;
import static star.vis.LutScale.LOG10;
import static star.vis.VisProjectionMode.PARALLEL;
...
public class reactor extends StarMacro {
    ...
    private static final String RADIUSINPUT = "radius.input";

    private static final double PIPE_RADIUS = readDoubleFromFile(RADIUSINPUT);
    private static final double PIPE_WALLTHICKNESS = 0.01;  // Added onto PIPE_RADIUS

    private final double[] inletComponentfractionsVector = new double[]{0.05, 0.1, 0.1, 0.75};

    // Simulation Parameters
    private static final double PIPE_SAFEDISTANCE = 0.2 * PIPE_RADIUS;
    private static final int SIM_MINSTEPS = 2000;
    private static final int SIM_MAXSTEPS_ALLOWED = 10000;
    private final double MESH_BASESIZE = 0.001001;
    ...
    private void setCADpart() {
        ...
        star.cadmodeler.Body cadmodelerBody_Outlet = param_cadModel.getBody("Base_Outlet");
        star.cadmodeler.Body cadmodelerBody_Inlet = param_cadModel.getBody("Base_Inlet");
        star.cadmodeler.Body cadmodelerBody_Base = param_cadModel.getBody("Base");
    ...
    private void cadModel_Base_Inlet(Simulation sim) {
        ...
        PointSketchPrimitive inlet_SketchPoint1 = inlet_sketch.createPoint(new DoubleVector(inletCornerPoints_NE));
        PointSketchPrimitive inlet_SketchPoint2 = inlet_sketch.createPoint(new DoubleVector(inletCornerPoints_SE));
        PointSketchPrimitive inlet_SketchPoint3 = inlet_sketch.createPoint(new DoubleVector(inletCornerPoints_SW));
        PointSketchPrimitive inlet_SketchPoint4 = inlet_sketch.createPoint(new DoubleVector(inletCornerPoints_NW));
        ...
        Face face_inlet_inlet = extrusionMerge_inlet.getSideFace(inlet_SketchLine2, "True");
        Face face_inlet_wall = extrusionMerge_inlet.getSideFace(inlet_SketchLine1, "True");
        Face face_inlet_sym = extrusionMerge_inlet.getSideFace(inlet_SketchLine3, "True");
        ...
        star.cadmodeler.Body cadmodelerBody_inlet = extrusionMerge_inlet.getBody(inlet_SketchLine1);
    ...
    private void cadModel_Base_Outlet(Simulation sim) {
        ...
        PointSketchPrimitive outlet_SketchPoint1 = outlet_sketch.createPoint(new DoubleVector(outletCornerPoints_NE));
        PointSketchPrimitive outlet_SketchPoint2 = outlet_sketch.createPoint(new DoubleVector(outletCornerPoints_SE));
        PointSketchPrimitive outlet_SketchPoint3 = outlet_sketch.createPoint(new DoubleVector(outletCornerPoints_SW));
        PointSketchPrimitive outlet_SketchPoint4 = outlet_sketch.createPoint(new DoubleVector(outletCornerPoints_NW));
        ...
        // Naming the inlets faces
        Face face_outlet_outlet = extrusionMerge_outlet.getSideFace(outlet_SketchLine2, "True");
        Face face_outlet_sym = extrusionMerge_outlet.getSideFace(outlet_SketchLine1, "True");
        Face face_outlet_wall = extrusionMerge_outlet.getSideFace(outlet_SketchLine3, "True");
        ...
        star.cadmodeler.Body cadmodelerBody_inlet = extrusionMerge_outlet.getBody(outlet_SketchLine1);
    ...
    private void cadModel_Base(Simulation sim) {
        ...
        PointSketchPrimitive base_SketchPoint1 = base_sketch.createPoint(new DoubleVector(mainCornerPoint_SW));
        PointSketchPrimitive base_SketchPoint2 = base_sketch.createPoint(new DoubleVector(mainCornerPoint_SE));
        PointSketchPrimitive base_SketchPoint3 = base_sketch.createPoint(new DoubleVector(mainCornerPoint_NE));
        PointSketchPrimitive base_SketchPoint4 = base_sketch.createPoint(new DoubleVector(mainCornerPoint_NW));
        ...
        Face face_base_base = extrusionMerge_base.getSideFace(base_SketchLine1, "True");
        Face face_base_wall1 = extrusionMerge_base.getSideFace(base_SketchLine2, "True");
        Face face_base_wall2 = extrusionMerge_base.getSideFace(base_SketchLine3, "True");
        Face face_base_wall3 = extrusionMerge_base.getSideFace(base_SketchLine4, "True");
        ...
        star.cadmodeler.Body cadmodelerBody_inlet = extrusionMerge_base.getBody(base_SketchLine1);
    ...
    private void cadModel_Pipes(List<double[]> pipe_centers, Simulation sim, CadModel param_cadModel) {
        ...
        while (oc_pipes.hasNext()) {
            ...
            CircleSketchPrimitive circleSketchPrimitive_0 = sketch.createCircle(new DoubleVector(coords), PIPE_RADIUS + PIPE_WALLTHICKNESS);
            ...
            // name Body
            star.cadmodeler.Body cadmodelerBody_0 = em.getBody(circleSketchPrimitive_0);

            cadmodelerBody_0.setPresentationName("Pipe_" + counter);

            // name BC
            Face face_0 = em.getSideFace(circleSketchPrimitive_0, "True");
    ...
    private void cadModel_finalMerge(Simulation sim, CadModel param_cadModel) {
        ...
        targetbodies.add(param_cadModel.getBody("Base"));

        for (int i = 1; i < pipecenters.size() + 1; i++) {
            toolbodies.add(param_cadModel.getBody("Pipe_" + i));
        }
        ...
        // options
        subtractBodiesFeature.setKeepToolBodies(true);
    ...
    private void set2Dmesh() {
        ...
        msg("Starting to setup the Mesh.");
        ...
        for (star.cadmodeler.Body bdy : param_cadModel.getBodyManager().getBodies()) {

            param_cadModel.createParts(new NeoObjectVector(new Object[]{bdy}), "SharpEdges", 30.0, 4, false, 1.0E-5);
        ...
        // Creating region
        sim.getRegionManager().newRegionsFromParts(new NeoObjectVector(new Object[]{baseSolid}), "OneRegionPerPart", null, "OneBoundaryPerPartSurface", null, "OneFeatureCurve", null, BOUNDARY);
        ...
        meshOp.getDefaultValues().get(BaseSize.class).setValue(MESH_BASESIZE);

        NumPrismLayers numPrismLayers_0 = meshOpDefaults.get(NumPrismLayers.class);
        numPrismLayers_0.setNumLayers(30);
        ...
        prismThickness_0.setAbsoluteSize(0.08, units_m);
        ...
        surfaceCurvature_0.getCurvatureDeviationDistance().setValue(0.0025);
        ...
        prismAutoMesher_0.setConvexAngleLimit(280.0);
        ...
        MassFlowBoundary massFlowBoundary_0 = sim.get(ConditionTypeManager.class).get(MassFlowBoundary.class);
...
//    boundary_1.setBoundaryType(symmetryBoundary_0);
        ...
        PressureBoundary pressureBoundary_0 = sim.get(ConditionTypeManager.class).get(PressureBoundary.class);
        boundary_2.setBoundaryType(pressureBoundary_0);



        msg("Meshing setup done.");


    }
    
    
    private void compute2Dmesh() {
        
        // Preset boundaries for correct meshing
        Region reg = sim.getRegionManager().getRegion("Base");
        
        AutoMeshOperation2d meshOp = sim.get(MeshOperationManager.class).getObjectsOf(AutoMeshOperation2d.class).get(0);
    ...
    private void setPhysics() {
        ...
        gasMixture_0.getMaterialProperties().getMaterialProperty(ThermalConductivityProperty.class).setMethod(MoleFractionMixingThermalConductivityMethod.class);

        star.material.MaterialDataBase materialMaterialDataBase_0 = sim.get(MaterialDataBaseManager.class).getMatlDataBase("Standard");
        ...
        // Set temperature dependent Absorbtion Factor for Gas Mixture
        UserFieldFunction uFF_absorbH20
                = sim.getFieldFunctionManager().createFieldFunction();
        uFF_absorbH20.setPresentationName("absorpCoef_H2O");
        uFF_absorbH20.setFunctionName("absorpCoef_H2O");
        uFF_absorbH20.getTypeOption()
                .setSelected(SCALAR);
        uFF_absorbH20.setDefinition(
                "  +pow(${Temperature},13) * -4.028233989629028395e-38\n"
                + "+pow(${Temperature},12) *  7.713105448882742783e-34\n"
                + "+pow(${Temperature},11) * -6.681719084364786200e-30\n"
                + "+pow(${Temperature},10) *  3.463309593754725854e-26\n"
                + "+pow(${Temperature}, 9) * -1.196919999263041256e-22\n"
                + "+pow(${Temperature}, 8) *  2.909094247859089478e-19\n"
                + "+pow(${Temperature}, 7) * -5.109979148096756723e-16\n"
                + "+pow(${Temperature}, 6) *  6.561990685639154001e-13\n"
                + "+pow(${Temperature}, 5) * -6.156739520761451923e-10\n"
                + "+pow(${Temperature}, 4) *  4.168789420633076510e-07\n"
                + "+pow(${Temperature}, 3) * -1.982973323269064745e-04\n"
                + "+pow(${Temperature}, 2) *  6.303095176358891605e-02\n"
                + "+pow(${Temperature}, 1) * -1.214715193046499842e+01\n"
                + "+pow(${Temperature}, 0) *  1.112631330425216447e+03");

        UserFieldFunction uFF_absorbCO2 = sim.getFieldFunctionManager()
                .createFieldFunction();
        uFF_absorbCO2.setPresentationName("absorpCoef_CO2");
        uFF_absorbCO2.setFunctionName("absorpCoef_CO2");
        uFF_absorbCO2.getTypeOption()
                .setSelected(SCALAR);
        uFF_absorbCO2.setDefinition(
                "  +pow(${Temperature},13) *  3.737768535864315448e-38\n"
                + "+pow(${Temperature},12) * -6.996275777899036783e-34\n"
                + "+pow(${Temperature},11) *  5.894574874852335457e-30\n"
                + "+pow(${Temperature},10) * -2.951565257124855329e-26\n"
                + "+pow(${Temperature}, 9) *  9.764887716211211811e-23\n"
                + "+pow(${Temperature}, 8) * -2.243548298560778935e-19\n"
                + "+pow(${Temperature}, 7) *  3.659399649520199441e-16\n"
                + "+pow(${Temperature}, 6) * -4.250464502812830720e-13\n"
                + "+pow(${Temperature}, 5) *  3.464795210410685384e-10\n"
                + "+pow(${Temperature}, 4) * -1.909330391775579207e-07\n"
                + "+pow(${Temperature}, 3) *  6.586880765355056548e-05\n"
                + "+pow(${Temperature}, 2) * -1.199666888629465772e-02\n"
                + "+pow(${Temperature}, 1) *  6.075366215051521968e-01\n"
                + "+pow(${Temperature}, 0) *  9.207218355630996598e+01");

        UserFieldFunction uFF_absorbMix = sim.getFieldFunctionManager()
                .createFieldFunction();
        uFF_absorbMix.setPresentationName("absorpCoef_mix");
        uFF_absorbMix.setFunctionName("absorpCoef_mix");
        uFF_absorbMix.getTypeOption().setSelected(SCALAR);
        uFF_absorbMix.setDefinition("${AbsolutePressure}/101325*(${MoleFractionCO2}*${absorpCoef_CO2} +${MoleFractionH2O}*${absorpCoef_H2O})");

        AbsorptionCoefficientMaterialProperty absorpMaterial = gasMixture_0.getMaterialProperties()
                .getMaterialProperty(AbsorptionCoefficientMaterialProperty.class);

        absorpMaterial.setMethod(FunctionMaterialPropertyMethod.class);
        ((FunctionMaterialPropertyMethod) absorpMaterial.getMethod()).setFieldFunction(uFF_absorbMix);
...
//
        // MoleFractions
        phyConti.getInitialConditions()
                .get(SpeciesSpecificationOption.class
                ).setSelected(MOLE_FRACTION);
        MoleFractionProfile moleFractionProfile_0 = phyConti.getInitialConditions().get(MoleFractionProfile.class);

        moleFractionProfile_0.getMethod(ConstantArrayProfileMethod.class)
                .getQuantity().setArray(new DoubleVector(inletComponentfractionsVector));
        ...
        phyConti.getReferenceValues().get(MinimumAllowableTemperature.class).setValue(500.0);
        ...
        turbulenceIntensityProfile_0.getMethod(ConstantScalarProfileMethod.class
        ).getQuantity().setValue(0.1);
        ...
        velocityProfile_0.getMethod(ConstantVectorProfileMethod.class)
                .getQuantity().setComponents(0.1, 0.0, 0.0);

        // use TVB Gradient Limiter
        GradientsModel gradientsModel_0 = phyConti.getModelManager().getModel(GradientsModel.class);
        gradientsModel_0.setUseTVBGradientLimiting(true);
    ...
    private void setBC() {
        ...
        Boundary bndry_wall = reg.getBoundaryManager().getBoundary("WALL");
        bndry_wall.getValues().get(EmissivityProfile.class)
                .getMethod(ConstantScalarProfileMethod.class)
                .getQuantity().setValue(0.75);
        bndry_wall.getConditions().get(WallThermalOption.class)
                .setSelected(WallThermalOption.Type.ADIABATIC);
...
//
        bdry_inlet.getConditions().get(SpeciesSpecificationOption.class).setSelected(MOLE_FRACTION);
        ...
        radiationTemperatureProfile_1.getMethod(ConstantScalarProfileMethod.class).getQuantity().setValue(1100.0);
...
//
        bdry_outlet.getConditions().get(SpeciesSpecificationOption.class).setSelected(MOLE_FRACTION);
        ...
        String qTableName = "Q_of_T_" + PIPE_RADIUS;
        sim.getTableManager().createFromFile(resolvePath(qTableName + ".csv"));

        UserFieldFunction uFF_wallFlux = sim.getFieldFunctionManager().createFieldFunction();
        uFF_wallFlux.getTypeOption().setSelected(SCALAR);
        uFF_wallFlux.setDefinition("interpolateTable(@Table(\"" + qTableName + "\"), \"Temperature\", SPLINE, \"Heat Flux\",${Temperature})");
        ...
        for (Boundary bndry : boundaries) {
            if (bndry.getBoundaryType() instanceof WallBoundary) {
                if (bndry.getPresentationName().matches("^PWALL_.*")) {
                    bndry.getConditions().get(WallThermalOption.class).setSelected(HEAT_FLUX);
                    ...
                    bndry.getValues().get(EmissivityProfile.class)
                            .getMethod(ConstantScalarProfileMethod.class)
                            .getQuantity().setValue(0.6);
    ...
    private void setReports() {
        ...
        cartesian2DAxis_0.setMaximum(1800.0);
        ...
        ReportMonitor massFlowAverageReport_temperatureAtOutletMonitor = ((ReportMonitor) simMonMngr.getMonitor(massFlowAverageReport_temperatureAtOutlet.getPresentationName() + " Monitor"));
        ...
        ReportMonitor pDropReportMonitor = ((ReportMonitor) simMonMngr.getMonitor(pDropReport.getPresentationName() + " Monitor"));
        ...
        ReportMonitor heatTransferReport_BalanceMonitor = ((ReportMonitor) simMonMngr.getMonitor(heatTransferReport_Balance.getPresentationName() + " Monitor"));

        MonitorPlot monitorPlot_htBalance = simPlotMngr.createMonitorPlot(new NeoObjectVector(new Object[]{heatTransferReport_BalanceMonitor}), "Heat_Transfer_Balance_InletOutlet Plot");
        ...
        for (Boundary bndry : inoutList) {
            ...
            ReportMonitor enthalpyMonitor = ((ReportMonitor) simMonMngr.getMonitor(areaAverageReport.getPresentationName() + " Monitor"));
            MonitorPlot enthalphyMonitorPlot = simPlotMngr.createMonitorPlot(new NeoObjectVector(new Object[]{enthalpyMonitor}), areaAverageReport.getPresentationName() + " Plot");
    ...
    private void setSolverSettings() {
        ...
        CoupledImplicitSolver coupledImplicitSolver_0 = sim.getSolverManager().getSolver(CoupledImplicitSolver.class);
        coupledImplicitSolver_0.getRampCalculatorManager().getRampCalculatorOption().setSelected(LINEAR_RAMP);
        ...
        linearRampCalculator_0.setEndIteration(30);

        coupledImplicitSolver_0.setCFL(3.0);

//    coupledImplicitSolver_0.getExpertInitManager().getExpertInitOption().setSelected(ExpertInitOption.Type.GRID_SEQ_METHOD);
        // Radiation adatpion
        DORadiationSolver dORadiationSolver_0 = sim.getSolverManager().getSolver(DORadiationSolver.class);
        dORadiationSolver_0.setUrf(0.7);

        AMGLinearSolver aMGLinearSolver_0 = dORadiationSolver_0.getAMGLinearSolver();
        aMGLinearSolver_0.getCycleOption().setSelected(FLEX_CYCLE);
    ...
    private void setStoppingCriteria() {
        ...
        String[] stopCrits
                = {"Continuity", "X-momentum", "Y-momentum", "Energy", "Tdr", "Tke", "CO2", "H2O", "N2", "O2"};
        ...
        for (String stop : stopCrits) {
            ...
            itStpCrt.getLogicalOption().setSelected(AND);

            MonitorIterationStoppingCriterionMinLimitType itStpCrtLim
                    = ((MonitorIterationStoppingCriterionMinLimitType) itStpCrt
                            .getCriterionType());
        ...
        ReportMonitor reportMonitor_0
                = ((ReportMonitor) simMonMngr
                        .getMonitor("Heat_Transfer_Balance_InletOutlet Monitor"));
        MonitorIterationStoppingCriterion iterationStopCriterion
                = reportMonitor_0
                        .createIterationStoppingCriterion();

        iterationStopCriterion.getCriterionOption().setSelected(ASYMPTOTIC);
        MonitorIterationStoppingCriterionAsymptoticType monitorIterationStoppingCriterionAsymptoticType_0
                = ((MonitorIterationStoppingCriterionAsymptoticType) iterationStopCriterion
                        .getCriterionType());
        monitorIterationStoppingCriterionAsymptoticType_0.setNumberSamples(100);
        iterationStopCriterion.getLogicalOption().setSelected(AND);

        MonitorIterationStoppingCriterion monitorIterationStoppingCriterion_0
                = ((MonitorIterationStoppingCriterion) sim
                        .getSolverStoppingCriterionManager().getSolverStoppingCriterion("Heat_Transfer_Balance_InletOutlet Monitor Criterion"));

        monitorIterationStoppingCriterion_0
                .getCriterionOption().setSelected(RELATIVE_CHANGE);

        MonitorIterationStoppingCriterionRelativeChangeType monitorIterationStoppingCriterionRelativeChangeType_0
                = ((MonitorIterationStoppingCriterionRelativeChangeType) monitorIterationStoppingCriterion_0
                        .getCriterionType());
    ...
    private void setScenes() {
        ...
        velDsplr.getScalarDisplayQuantity().setRange(new DoubleVector(new double[]{0.0, 15.0}));
        velDsplr.getScalarDisplayQuantity().setClip(NONE);
        ...
        tmpDsplr.getScalarDisplayQuantity().setClip(NONE);

        //Temperature
        ScalarDisplayer tmpExtraDsplr = createNewScalarScene("Temperature_withColoredPipes");
        tmpExtraDsplr.getScalarDisplayQuantity().setFieldFunction(ff_tmp);
        tmpExtraDsplr.getScalarDisplayQuantity().setRange(new DoubleVector(new double[]{800.0, 1800.0}));
        tmpExtraDsplr.getScalarDisplayQuantity().setClip(NONE);
        ...
        incidentRadiatDsplr.getScalarDisplayQuantity().setClip(NONE);
        ...
        vorDsplr.getLegend().setScaleMode(LOG10);
        ...
        tkeDsplr.getLegend().setScaleMode(LOG10);
        ...
        tdrDsplr.getLegend().setScaleMode(LOG10);
        ...
        ScalarDisplayer absorpH2ODsplr = createNewScalarScene("absorpCoef_H2O");
        FieldFunction ff_absorpH2O = simFFMngr.getFunction("absorpCoef_H2O");
        absorpH2ODsplr.getScalarDisplayQuantity().setFieldFunction(ff_absorpH2O);

        //Absorption Coefficient mix
        ScalarDisplayer absorpmixDsplr = createNewScalarScene("absorpCoef_mix");
        FieldFunction ff_absorpmix = simFFMngr.getFunction("absorpCoef_mix");
        absorpmixDsplr.getScalarDisplayQuantity().setFieldFunction(ff_absorpmix);

        //Absorption Coefficient CO2
        ScalarDisplayer absorpCO2Dsplr = createNewScalarScene("absorpCoef_CO2");
        FieldFunction ff_absorpCO2 = simFFMngr.getFunction("absorpCoef_CO2");
        absorpCO2Dsplr.getScalarDisplayQuantity().setFieldFunction(ff_absorpCO2);
        ...
        currentView_geo.setInput(new DoubleVector(new double[]{0.0, 0.05000000000000002, 0.0}), new DoubleVector(new double[]{0.0, 0.05000000000000002, 0.4}), new DoubleVector(new double[]{0.0, 0.0, 1.0}), 0.20, 0);
        ...
        currentView_mesh.setInput(new DoubleVector(new double[]{0.0, 0.05000000000000002, 0.0}), new DoubleVector(new double[]{0.0, 0.05000000000000002, 0.4}), new DoubleVector(new double[]{0.0, 0.0, 1.0}), 0.20, 0);
...
//        msg("Returning " + scene.getPresentationName() + "'s " + scalarDisplayer.getPresentationName() + " Scalar Displayer");
    ...
    private void runSim() {
        ...
        PhysicsContinuum phyConti
                = ((PhysicsContinuum) sim.getContinuumManager().getContinuum("Physics 1"));

        msg("Running the Simulation.");

        try {
            phyConti.disableModel(phyConti.getModelManager().getModel(RadiationModel.class));
            phyConti.disableModel(phyConti.getModelManager().getModel(DORadiationModel.class));
            phyConti.disableModel(phyConti.getModelManager().getModel(ParticipatingGrayModel.class));
    ...
    private void saveScenes(String path) {
        ...
        colorizePipePatches(sim.getSceneManager().getScene("Temperature_withColoredPipes"));
    ...
    private void saveReportsValues() {
        ...
        for (Report rep : sim.getReportManager().getObjects()) {
            ...
            saveValueToFile(valueOf((Double) rep.monitoredValue()), "rep_" + rep.getPresentationName() + ".val");
        }
    }
    ...
    private static Double readDoubleFromFile(String valFile) {
        String line;
        Double value = null;
    ...
    private List<double[]> read_pipe_coordinates(String csvFile) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
            while ((line = br.readLine()) != null) {
                ...
                List<String> values = asList(line.split(cvsSplitBy));

                double[] currentrow = new double[2];

                switch (values.size()) {
                    case 1:
                        currentrow[0] = valueOf(values.get(0));
                        currentrow[1] = 0.0;
                        break;

                    case 2:
                        currentrow[0] = valueOf(values.get(0));
                        currentrow[1] = valueOf(values.get(1));
                        break;
...
//        sim.println("Naming sketch " + sketchname);
    ...
    private void arePipesIntersectingEachOther() {
        ...
        for (int i = 0; i < pipecenters.size(); i++) {
            for (int j = i + 1; j < pipecenters.size(); j++) {
                p_distance = sqrt(
                        pow(pipecenters.get(i)[0] - pipecenters.get(j)[0], 2)
                        + pow(pipecenters.get(i)[1] - pipecenters.get(j)[1], 2));

                if (p_distance < PIPE_SAFEDISTANCE + 2 * (PIPE_RADIUS + PIPE_WALLTHICKNESS)) {
    ...
    private void arePipesIntersectingWalls() {
        ...
        for (int i = 0; i < pipecenters.size(); i++) {
            bl = distancePointLine(pipecenters.get(i), mainCornerPoint_SW, mainCornerPoint_NW) < PIPE_SAFEDISTANCE + PIPE_RADIUS + PIPE_WALLTHICKNESS;
            bt = distancePointLine(pipecenters.get(i), mainCornerPoint_NW, mainCornerPoint_NE) < PIPE_SAFEDISTANCE + PIPE_RADIUS + PIPE_WALLTHICKNESS;
            br = distancePointLine(pipecenters.get(i), mainCornerPoint_NE, mainCornerPoint_SE) < PIPE_SAFEDISTANCE + PIPE_RADIUS + PIPE_WALLTHICKNESS;
            bb = distancePointLine(pipecenters.get(i), mainCornerPoint_SE, mainCornerPoint_SW) < PIPE_SAFEDISTANCE + PIPE_RADIUS + PIPE_WALLTHICKNESS;
    ...
    private void arePipesInsideDomain() {
        ...
        for (int i = 0; i < pipecenters.size(); i++) {
            ...
            if (bl || br || bt || bb) {
                msgErr("Pipe_" + (i + 1) + " is not inside the domain. "
                        + "Left:" + bl + " Top:" + bt
                        + " Right:" + br + " Bottom:" + bb
                );
    ...
    private static double distancePointLine(double[] point, double[] line_point1, double[] line_point2) {
        double p0x = point[0];
        double p0y = point[1];
        ...
        double num = abs((p2y - p1y) * p0x - (p2x - p1x) * p0y + p2x * p1y - p2y * p1x);

        double den = sqrt(pow(p2y - p1y, 2) + pow(p2x - p1x, 2));

        return num / den;
    }

    // checking if point is left of a line (given by two points)
    private static boolean isPointLeftOfLine(double[] point, double[] line_point1, double[] line_point2) {
        double p0x = point[0];
        double p0y = point[1];
        double p1x = line_point1[0];
        double p1y = line_point1[1];
    ...
    private static Object[] appendObject(Object[] obj, Object newObj) {
        ArrayList<Object> temp = new ArrayList<>(asList(obj));
        temp.add(newObj);
    ...
    private void saveReportValueToFile(String reportName, String fileName) {
        ...
        String repVal = valueOf((Double) rep.monitoredValue());
    ...
    private void fitViewOfScene(Scene sce) {
        sce.getCurrentView().setProjectionMode(PARALLEL);
        ...
        if (displayer != null) {
            Collection<NamedObject> pg = sce
                    .getDisplayerManager().getDisplayer("Scalar 1").getInputParts().getParts();
    ...
    private void colorizePipePatches(Scene scene) {

        Simulation sim = getActiveSimulation();

        PartRepresentation geometryRepresentation
                = ((PartRepresentation) sim.getRepresentationManager().getObject("Geometry"));

        for (int i = 1; i < pipecenters.size() + 1; i++) {
            String pipeName = "PWALL_" + i;
            String pipePartName = "Pipe_" + i;

            SolidModelPart solidModelPart_2 = ((SolidModelPart) sim.get(SimulationPartManager.class).getPart(pipePartName));
            PartSurface partSurface_0 = ((PartSurface) solidModelPart_2.getPartSurfaceManager().getPartSurface("FLUID"));

            PartDisplayer geom = (PartDisplayer) scene.getDisplayerManager().hasObject(pipeName);
            if (geom == null) {
                geom = scene.getDisplayerManager().createPartDisplayer("Geometry", -1, 4);
                geom.setPresentationName(pipeName);
            }
            geom.setSurface(true);
            geom.setRepresentation(geometryRepresentation);
            geom.getInputParts().setQuery(null);
            geom.getInputParts().setObjects(partSurface_0);

            geom.setColorMode(PartColorMode.CONSTANT);

            AreaAverageReport tempReport = ((AreaAverageReport) sim.getReportManager().getReport("Temperature_" + pipeName));

            geom.setDisplayerColorColor(getColorBlueRed(tempReport.getValue(), 800.0, 1800.0));

        }

    }

    private static Color getColorBlueRed(double value, double minValue, double maxValue) {
        float hueMin = 0f;
        float hueMax = 0.666666667f;
        ...
        double relativePosition = (value - minValue) / (maxValue - minValue);
        float hue = (hueMax - hueMin) * (1f - (float) relativePosition) + hueMin;

        return Color.getHSBColor(hue, 1.0f, 1.0f);
    }

//    private void initialAnnotations() {
//        // Helper class
//        class AnnotationsWrapper {
//
//            public AnnotationsWrapper(String annotationName, String labelText, double[] defaultPositionVector, double textHeight) {
//                this.name = annotationName;
//                this.pos = new DoubleVector(defaultPositionVector);
//                this.text = labelText;
//                this.height = textHeight;
//            }
//            public String name;
//            public DoubleVector pos;
//            public String text;
//            public double height;
//        }
//
//        // preparing settings
//        List<AnnotationsWrapper> annotations = new ArrayList<>();
//        annotations.add(new AnnotationsWrapper("Label_Inlet", "Inlet - scaled by 1.5", new double[]{0.01, 0.625, 0.5}, 0.025));
//        annotations.add(new AnnotationsWrapper("Label_Outlet", "Outlet - scaled by 1.5", new double[]{0.95, 0.39, 0.5}, 0.025));
//        annotations.add(new AnnotationsWrapper("Label_Rad", "Tube radius: " + PIPE_RADIUS + " m", new double[]{0.01, 0.01, 0.0}, 0.02));
//
//        // Create with prepared settings
//        Simulation sim = getActiveSimulation();
//        for (AnnotationsWrapper ann : annotations) {
//            SimpleAnnotation simpleAnnotation = sim.getAnnotationManager().createSimpleAnnotation();
//
//            simpleAnnotation.setPresentationName(ann.name);
//            simpleAnnotation.setText(ann.text);
//            simpleAnnotation.setDefaultHeight(ann.height);
//            simpleAnnotation.setDefaultPosition(ann.pos);
//
//            simpleAnnotation.setFontString("Dialog-Plain");
//            simpleAnnotation.setShadow(false);
//
//        }
//
//    }
